...
```
Main / Caller                    Timer / Sleep
    │                                │
    │──── schedule "A" (2s) ────────>│
    │                                │
    │ print "B" (ทันที)                │  … รอ 2 วินาที …
    │                                │
    │<──── callback / resume ────────│
    │                                │
    │ print "A"                      │
...
│ • print A                           │
│ • await sleep 2s                    │
│ • print B (หลัง sleep เสร็จ)          │
└─────────────────────────────────────┘
            │
ระหว่าง await → yield ให้ loop รัน
            ▼
┌─────────────────────────────────────┐
│  บรรทัดถัดจาก main()                  │
├─────────────────────────────────────┤
...
```
┌──────────────┐     ┌─────────────────────────────┐
│  Call Stack  │<───>│ Web APIs / timers / network │
└──────┬───────┘     └─────────────┬───────────────┘
       ▲                           │
       │                           ▼
┌──────┴───────────────────────────┴──────┐
